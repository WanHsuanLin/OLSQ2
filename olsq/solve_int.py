...
import datetime
import itertools
from z3 import Int, IntVector, Bool, Implies, And, Or, sat, unsat, Solver, set_option, BitVec, ULT, ULE, UGE, BitVecVal, PbLe, Not, Function, BitVecSort, Distinct, BoolSort, AtMost, Then
...
class OLSQ:
    ...
    def solve(self, use_sabre, output_mode: str = None, output_file_name: str = None, dump_smt_file = False, memory_max_size=MEMORY_MAX_SIZE, verbose = VERBOSE):
        ...
        if self.mode == Mode.transition:
            ...
            results = self._solve(use_sabre, output_mode, output_file_name, dump_smt_file, memory_max_size, verbose)
    ...
    def _solve(self, use_sabre = False, output_mode: str = None, output_file_name: str = None, dump_smt_file = False, memory_max_size=MEMORY_MAX_SIZE, verbose = VERBOSE):
        ...
        if self.mode == Mode.transition:
            ...
            bound_depth = max(len(self.list_gate_name) // 10, 5)
        else:
        ...
        while not_solved:
            ...
            self._add_transformation_constraints(bound_depth, lsqc, sigma, pi)
            #d = 9
            #lsqc.add([UGE(d, time[l]) for l in range(len(self.list_gate_qubits))])
            #constraints = lsqc.sexpr()
            #f = open("dump.txt", "w")
            #f.write(constraints)
            #f.write("(check-sat)")
            ...
            if self.mode == Mode.transition:
                not_solved, model = self._optimize_circuit_tran(lsqc , time, sigma, count_gate, bound_depth)
            else:
                not_solved, model = self._optimize_circuit_normal(iteration, lsqc , time, sigma, count_gate, bound_depth, use_sabre, dump_smt_file)
            if not_solved:
                bound_depth *= 2
                iteration += 1
            else:
    ...
    def _add_injective_mapping_constraints(self, bound_depth, pi, model):
        ...
        for t in range(bound_depth):
            ...
            for m in range(self.count_program_qubit):
                model.add(pi[m][t] >= 0)
                model.add(pi[m][t] < self.count_physical_qubit)
                for mm in range(m):
